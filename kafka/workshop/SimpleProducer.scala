  ...
  def main(args: Array[String]): Unit = {
    ...
    for (i <- 0 until 5000) {
      for (message <- greetingMessages) {
        ...
        val metadata: RecordMetadata = producer.send(record).get() // sync, blocking

        printf("Greeting %d - %s sent\n", counter, message)
        println("Ack offset " + metadata.offset() + " partition " + metadata.partition())
